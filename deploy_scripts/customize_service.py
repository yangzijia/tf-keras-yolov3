...
###ep039-loss8.315-val_loss8.070.h5
class ObjectDetectionService(TfServingBaseService):
    def __init__(self, model_name, model_path):
        ...
        self.anchors_path = os.path.join(os.path.dirname(__file__), 'yolo_anchors.txt')
    ...
    def _inference(self, data):
        ...
        if out_boxes is not None:
            ...
            for box in out_boxes:
                ...
                out_boxes_list.append([round(float(v), 1) for v in res_box])  # v是np.float32类型，会导致无法json序列化，因此使用float(v)转为python内置float类型
